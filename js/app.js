...
function resolveUrl(x) {
  ...
  // URLs web continuam sendo abertas normalmente.
  if (/^https?:\/\//i.test(u)) return u;

  // Normaliza os caminhos da rede para UNC.
  // Ex.: file:///\\arquivos\ti\G_Xanxere_TI\... -> \\arquivos\ti\G_Xanxere_TI\...
  if (/^file:\/\//i.test(u)) {
  ...
  // Caminho UNC já existente.
  if (/^\\\\/.test(u)) {
    return u.replace(/\//g, '\\');
  }

  // Caminho de unidade mapeada (G:\...).
  if (/^[A-Za-z]:[\\/]/.test(u)) {
    return u.replace(/\//g, '\\');
  }

  // Caminho relativo: mantém a configuração existente como fallback.
  const cfg = window.PORTAL_CONFIG || {};
  let root = String(cfg.networkRoot || '').trim();

  if (root) {
    root = root.replace(/^file:\/\//i, '').replace(/\//g, '\\');
    root = root.replace(/^\\+/, '').replace(/\\+$/, '');
    let cleanRel = u.replace(/^[/\\]+/, '').replace(/\//g, '\\');
...
function openItem(x) {
  const target = resolveUrl(x);
  if (!target) {
    toast('Este atalho ainda precisa de uma URL ou caminho válido.');
    return;
  }
  ...
  // Para arquivos da rede, usamos um <a> real com o caminho UNC,
  // em vez de window.open(). URLs HTTP/HTTPS também usam navegação nativa.
  // Isso permite testar o comportamento do navegador sem criar scripts,
  // extensões ou componentes locais.
