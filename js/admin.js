...
function showLogin() {
  ...
  const lp = $('#loginPanel');
  const ap = $('#adminPanel');
  if (lp) { lp.hidden = false; lp.style.display = 'block'; }
...
function showAdmin() {
  ...
  const lp = $('#loginPanel');
  const ap = $('#adminPanel');
  if (lp) { lp.hidden = true; lp.style.display = 'none'; }
...
function renderCategories() {
  ...
  if (A.categories.length === 0) {
    tbody.innerHTML = '<tr><td colspan="2" style="text-align:center; padding:1.5rem;">Nenhuma categoria encontrada.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = A.categories.map(c => `
    <tr>
      <td>
        ...
        ${c.description ? `<br><small style="opacity:0.7">${esc(c.description)}</small>` : ''}
      </td>
...
function renderLinks() {
  ...
  if (countEl) {
    countEl.textContent = `${filtered.length} de ${A.links.length} ${A.links.length === 1 ? 'atalho' : 'atalhos'}`;
  }

  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:1.5rem;">Nenhum atalho encontrado.</td></tr>';
    return;
  }
...
function populateCategoryDropdown(selectedCatIdOrName) {
  ...
  if (A.categories.length === 0) {
    sel.innerHTML = '<option value="">Nenhuma categoria cadastrada</option>';
    return;
  }

  sel.innerHTML = A.categories.map(c => {
    const isSelected = String(selectedCatIdOrName) === String(c.id) || selectedCatIdOrName === c.name;
    return `<option value="${c.id}" ${isSelected ? 'selected' : ''}>${esc(c.icon || '📁')} ${esc(c.name)}</option>`;
  }).join('');
}
...
function openEditor(x = null) {
  ...
  populateCategoryDropdown(x?.category_id || x?.category);
  showModal($('#editor'));
}
...
async function loadAdmin() {
  if (localMode()) {
    ...
    if (info) info.textContent = 'Modo Local (Gerenciamento no navegador / LocalStorage)';
    refreshLocal();
    return;
  }
  ...
  showAdmin();
  const info = $('#sessionInfo');
  if (info) info.textContent = `Administrador: ${A.user.email}`;
  await refresh();
}

// Event Listeners dos Modais
$('#newLink')?.addEventListener('click', () => openEditor());
$('#closeEditor')?.addEventListener('click', closeEditor);
$('#cancelEditor')?.addEventListener('click', closeEditor);

$('#newCategory')?.addEventListener('click', () => openCategoryModal());
...
// Pesquisa dinâmica na administração
...
$('#linkForm')?.addEventListener('submit', async e => {
  ...
  const catObj = A.categories.find(c => String(c.id) === String(catId));
    ...
    if (id) {
      ...
      if (idx !== -1) {
        A.links[idx] = { 
          ...A.links[idx], 
          ...payload, 
          category_name: catObj ? catObj.name : 'Sem categoria' 
        };
      }
    } else {
      payload.id = String(Date.now());
      payload.category_name = catObj ? catObj.name : 'Sem categoria';
...
// Delegação de eventos globais para Edição e Exclusão
document.addEventListener('click', async e => {
    ...
    if (confirm('Deseja realmente excluir este atalho?')) {
      ...
      if (localMode()) {
        ...
        if (error) {
          ...
          await refresh();
          toast('Atalho excluído com sucesso.');
        }
      }
    }
    return;
  ...
  if (delCat) {
    const catId = delCat.dataset.deleteCat || delCat.getAttribute('data-delete-cat');
    const hasLinks = A.links.some(x => String(x.category_id) === String(catId) || String(x.category) === String(catId));
    if (hasLinks) {
      alert('Não é possível excluir uma categoria que possui atalhos vinculados. Remova os atalhos primeiro.');
      return;
    }
    if (confirm('Deseja realmente excluir esta categoria?')) {
      if (localMode()) {
        A.categories = A.categories.filter(c => String(c.id) === String(catId));
        localStorage.setItem('pti_local_cats', JSON.stringify(A.categories));
        refreshLocal();
        toast('Categoria excluída (Modo Local).');
      } else {
        const { error } = await portalSupabase.from('categories').delete().eq('id', catId);
        if (error) {
          toast(error.message);
        } else {
          await refresh();
