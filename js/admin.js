...
* Normaliza o caminho informado pelo administrador.
*
* Exemplos aceitos:
*
* $$arquivos\ti\G_Xanxere_TI\Documentacao\arquivo.xlsx
  $$
*
* file://arquivos/ti/G_Xanxere_TI/Documentacao/arquivo.xlsx
*
* C:\TI\arquivo.xlsx
*
...
* Caminho relativo.
*
* Não alteramos para URL absoluta aqui.
* O app.js continuará responsável pelo fallback
* utilizando PORTAL_CONFIG.networkRoot.
  */
  return url;
  }
...
const localData =
window.portalData ||
(typeof portalData !== 'undefined' ? portalData : null);

const savedCats =
JSON.parse(localStorage.getItem('pti_local_cats') || 'null');

const savedLinks =
JSON.parse(localStorage.getItem('pti_local_links') || 'null');
...
tbody.innerHTML = A.categories.map(c => ` <tr> <td> <strong>
${esc(c.icon || '📁')}
${esc(c.name)} </strong>
...
tbody.innerHTML = filtered.map(x => ` <tr> <td> <strong>${esc(x.name)}</strong> </td>
...
* O campo mostra o caminho real cadastrado.
*
* Exemplo:
...
/*
 * Normaliza o endereço antes de salvar.
 */
const originalUrl =
  $('#linkUrl').value.trim();

const normalizedUrl =
  normalizeLinkUrl(originalUrl);
...
if (!payload.url) {
  ...
  $('#formMsg').textContent =
    'Informe o caminho ou URL do arquivo.';

  return;
}

/*
 * Se o usuário digitou um caminho UNC,
 * mostramos o caminho que será salvo.
 */
if (
  ...
  $('#formMsg').textContent =
    `Caminho normalizado para: ${normalizedUrl}`;
...
if (localMode()) {
  ...
  toast(
    'Atalho salvo no navegador (Modo Local).'
  );
...
const delCat =
  e.target.closest('[data-delete-cat]');
